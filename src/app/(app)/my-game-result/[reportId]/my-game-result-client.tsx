...
import { ArrowLeft, Award, CheckCircle2, XCircle, MinusCircle, Medal, Trophy, Gamepad2, HelpCircle } from "lucide-react"
...
import { getMyGameResult, getMyGames, type IMyGameResultQuestion } from "@/api/reports"

function getRankLabel(rank: number) {
  if (rank === 1) return { icon: Trophy, label: "1 место", color: "text-amber-300" }
  if (rank === 2) return { icon: Medal, label: "2 место", color: "text-slate-200" }
  if (rank === 3) return { icon: Medal, label: "3 место", color: "text-orange-300" }
  return { icon: Award, label: `${rank} место`, color: "text-white/50" }
}

function QuestionRow({ q, index }: { q: IMyGameResultQuestion; index: number }) {
            ...
            {q.element_effects.map((eff, i) => (
              <span
                ...
                className={cn(
                  ...
                  eff.points >= 0 ? "bg-emerald-500/15 text-emerald-400" : "bg-red-500/15 text-red-400",
                )}
              >
                {eff.points >= 0 ? "+" : ""}
      ...
      <div className="shrink-0">
        <span
          className={cn(
            ...
            isRight ? "text-emerald-400" : isWrong ? "text-red-400" : "text-white/25",
          )}
        >
          <span>
            {q.points > 0 ? "+" : ""}
...
export default function MyGameResultClient({ reportId }: { reportId: string }) {
  const {
    data: result,
    isLoading,
    isError,
  ...
  if (isLoading) {
    return (
      <main className="text-foreground flex h-full min-h-0 w-full flex-col items-center overflow-y-auto px-0">
        <section className="container mx-auto w-full max-w-5xl px-4 pt-6 pb-10">
          <PageSkeleton />
        </section>
      </main>
    )
  ...
  if (isError || !result) {
    return (
      <main className="text-foreground flex h-full min-h-0 w-full flex-col items-center overflow-y-auto px-0">
        <section className="container mx-auto flex w-full max-w-5xl flex-col items-center gap-4 px-4 py-12">
          <HelpCircle className="size-12 text-white/20" />
          <p className="text-center text-sm text-white/40">Не удалось загрузить результат игры</p>
  ...
  return (
    <main className="text-foreground flex h-full min-h-0 w-full flex-col items-center overflow-y-auto px-0">
      <section className="container mx-auto flex w-full max-w-5xl flex-col gap-0 px-4 pt-6 pb-10">
        <Link
          href="/"
        ...
        >
          <Button variant="ghost" size="icon-sm" aria-hidden tabIndex={-1} className="pointer-events-none">
        ...
        <div className="relative mb-5 overflow-hidden">
          ...
          <div className={cn("relative flex flex-col gap-3", imageUrl ? "-mt-12 px-1" : "")}>
            {!imageUrl && (
            ...
            <div className="min-w-0">
              <h2 className="text-xl font-bold leading-tight text-balance text-white sm:text-2xl">{quizName}</h2>
              {quizDescription && <p className="mt-1.5 line-clamp-3 text-sm leading-relaxed text-white/50">{quizDescription}</p>}
            ...
            <div className="mt-1 grid grid-cols-3 gap-2 sm:gap-3">
              <div className="flex flex-col items-center gap-1 rounded-xl bg-white/5 py-3">
                ...
                <span className={cn("text-sm font-bold tabular-nums", rank.color)}>{rank.label}</span>
                <span className="text-[10px] text-white/30">Место</span>
              </div>

              <div className="flex flex-col items-center gap-1 rounded-xl bg-white/5 py-3">
                <PickaxeIcon points={result.total_points} className="size-5" />
                <span className={cn("text-sm font-bold tabular-nums", result.total_points >= 0 ? "text-white/90" : "text-red-400")}>
                  {result.total_points}
                </span>
        ...
        {result.questions.length > 0 && (
          <div className="flex flex-col gap-1">
            <p className="mb-2 px-1 text-xs font-medium tracking-wide text-white/30 uppercase">Вопросы</p>
