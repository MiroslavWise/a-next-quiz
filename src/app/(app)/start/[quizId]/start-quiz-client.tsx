...
export default function StartQuizClient({ quizId }: { quizId: string }) {
  ...
  const { lastByType, isConnected, connectSeq } = useQuizSocketIO({
    reportId: quizId,
  ...
  return (
    <div className="flex h-full w-full flex-col items-center justify-center gap-2 px-4 pb-12 text-center">
      <p className="text-base font-medium text-white/80">Игра недоступна</p>
      <p className="text-sm text-white/45">Неизвестный статус сессии или данные ещё не загружены.</p>
    </div>
  )
}
