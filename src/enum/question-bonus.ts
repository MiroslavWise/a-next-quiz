...
const QUESTION_BONUS_DETAILS: Record<QuestionBonus, string> = {
  ...
  [QuestionBonus.ALL_ELEMENTS_BOOST]:
    "Усиливает положительные параметры огня, воды, земли и воздуха на этом вопросе (недостатки и аватар без изменений). Огонь: speed ×1.22, первый верный +13% base. Вода: +8% к ответу, +7×N. Земля: любой верный +15% base при ответе (классическое «терпение» при END отключено). Воздух: порыв 55% → +9%, 25% → +18% base.",
...
export const ALL_ELEMENTS_BOOST_COMPARISON: readonly AllElementsBoostRow[] = [
  ...
  },
  {
    ...
    accentColor: "#06B6D4",
    normal: "Верный +5% к ответу; +5×N; недостаток Водоворот 16% → −18% и пул",
    boosted: "+8%; +7×N; −6×N",
  },
  {
    name: "Земля",
